...
export const RugbyAdmin = ({ match }: { match: Match }) => {
  ...
  const handleSetup = async () => {
      try {
        await RugbyLogic.setupTeams(match, homePlayers, awayPlayers, homeColor, awayColor);
        setInitMode(false);
      } catch (err: any) {
  ...
  const handleColorUpdate = async (isHome: boolean, color: string) => {
      ...
      if (!initMode) {
          await RugbyLogic.updateColors(match, isHome ? color : (match.homeTeamColor || '#000000'), !isHome ? color : (match.awayTeamColor || '#ffffff'));
      }
  };
    ...
    <div className="bg-white">
        ...
        <div className="bg-gray-50 border border-gray-200 p-4 mb-6 flex justify-between items-center relative">
             ...
             <input 
                ...
                className="absolute top-2 left-2 w-4 h-4 p-0 border-0 opacity-10 hover:opacity-100 cursor-pointer"
             ...
             <input 
                ...
                className="absolute top-2 right-2 w-4 h-4 p-0 border-0 opacity-10 hover:opacity-100 cursor-pointer"
                title="Change Away Color"
             />
