...
export default function App() {
  ...
  // Filter State
  const [sportFilter, setSportFilter] = useState<'all' | 'cricket' | 'netball' | 'rugby' | 'football'>('all');
  ...
  let heroPool: Match[] = [];
  if (liveMatches.length > 0) heroPool = liveMatches;
  else if (upcomingMatches.length > 0) heroPool = upcomingMatches;
  else heroPool = matches.slice(0, 3);
            ...
            <div className="relative z-10 w-full max-w-md">
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 p-8 md:p-12 shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="flex flex-col items-center mb-10 opacity-0 animate-fade-in-down">
                         ...
                         <h1 className="text-3xl font-display font-bold text-white uppercase tracking-tight text-center leading-none mb-2">Match<span className="text-penrice-gold">Centre</span></h1>
                    ...
                    <form onSubmit={handleAppUnlock} className="space-y-6 opacity-0 animate-fade-in-up delay-200">
                        <div className="relative group">
                            ...
                            <input type="password" value={passwordInput} onChange={(e) => { setPasswordInput(e.target.value); setUnlockError(false); }} placeholder="Enter Access Code" className="relative w-full bg-neutral-900/80 border border-white/10 text-center text-white placeholder-gray-600 font-bold text-lg py-4 outline-none focus:border-penrice-gold/50 focus:bg-neutral-900 transition-all rounded-sm" autoFocus />
                        </div>
                        {unlockError && (<div className="text-center animate-bounce"><span className="inline-block bg-red-500/10 border border-red-500/20 text-red-500 text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-sm">Access Denied</span></div>)}
                        <button type="submit" className="w-full bg-white text-black font-display font-bold text-sm uppercase tracking-widest py-4 hover:bg-penrice-gold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg">Connect to Dashboard</button>
                    </form>
                    ...
                    <div key={currentHeroMatch.id} className="absolute inset-0 animate-fade-in">
                        ...
                        <div className="absolute inset-0 flex flex-col justify-center items-center p-8 md:p-16 text-center z-10">
                            ...
                            <div className="flex flex-col gap-2 md:gap-4 w-full max-w-4xl">
                                <h1 className="font-display font-bold text-5xl md:text-8xl lg:text-9xl uppercase tracking-tighter leading-[0.85] text-white drop-shadow-2xl opacity-90 truncate">{currentHeroMatch.teamName}</h1>
                                ...
                                <h1 className="font-display font-bold text-5xl md:text-8xl lg:text-9xl uppercase tracking-tighter leading-[0.85] text-white drop-shadow-2xl opacity-90 truncate">{currentHeroMatch.opponent}</h1>
                            </div>
                        ...
                        {heroCount > 1 && (
                            <div className="absolute bottom-8 left-0 right-0 flex justify-center gap-3 z-20">
                                {Array.from({ length: heroCount }).map((_, i) => (
                                    <button key={i} onClick={() => setHeroIndex(i)} className={`h-1 transition-all duration-300 ${i === heroIndex ? 'w-8 bg-penrice-gold' : 'w-2 bg-white/20 hover:bg-white/50'}`} />
                                ))}
                            </div>
                        )}
                    </div>
                ) : (
                    <div className="absolute inset-0 flex items-center justify-center bg-black"><span className="text-gray-600 font-bold uppercase tracking-widest text-xs">No Featured Matches</span></div>
                )}
            </div>
            <div className="relative w-full md:w-[30%] h-[40vh] md:h-full bg-white border-l border-gray-100 flex flex-col z-20 shadow-2xl">
                 ...
                 <div className="flex-1 overflow-y-auto custom-scroll p-0">
                     {secondaryMatches.length === 0 ? (
                         <div className="p-8 text-center text-gray-400 text-xs font-bold uppercase tracking-widest italic">No other fixtures scheduled today.</div>
                     ) : (
                         secondaryMatches.map((m, i) => (
                             <div key={m.id} className="group border-b border-gray-100 p-6 hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setShowWelcome(false)}>
                                 <div className="flex justify-between items-start mb-2">
                                     ...
                                     <span className={`text-[9px] font-bold uppercase ${m.status === 'LIVE' ? 'text-red-600 animate-pulse' : 'text-gray-400'}`}>{m.status === 'UPCOMING' ? m.time || 'Today' : m.status}</span>
                                 </div>
                                 <div className="flex justify-between items-center">
                                    ...
                                    <div className="text-xl font-display font-bold text-gray-300 group-hover:text-black transition-colors">{m.status === 'UPCOMING' ? 'VS' : `${m.homeScore}-${m.awayScore}`}</div>
                                 </div>
                             </div>
                         ))
                     )}
                 ...
                 <div className="p-6 md:p-8 border-t border-gray-100 bg-white">
                     <button onClick={() => setShowWelcome(false)} className="w-full bg-black text-white py-4 md:py-5 font-display font-bold text-lg uppercase tracking-widest hover:bg-penrice-gold hover:text-black transition-all flex items-center justify-center group">
                         Enter Match Centre
                         <i className="fa-solid fa-arrow-right ml-3 text-sm group-hover:translate-x-1 transition-transform"></i>
                     </button>
                     <p className="text-center text-[9px] font-bold text-gray-300 uppercase tracking-widest mt-4">Penrice Academy Official Sport</p>
                 </div>
            </div>
        </div>
      );
      ...
      <header className="bg-white border-b border-black sticky top-0 z-[60] shadow-sm">
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-center h-auto md:h-24">
           ...
           <div className="flex items-center w-full md:w-auto border-t md:border-t-0 md:border-l border-gray-100 h-full">
               {!isAdmin && (
                   <>
                       <button onClick={() => setViewMode('live')} className={`flex-1 md:flex-none h-12 md:h-full px-6 text-[10px] font-bold uppercase tracking-widest border-r border-gray-100 transition-colors ${viewMode === 'live' ? 'bg-black text-white' : 'text-gray-500 hover:text-black hover:bg-gray-50'}`}>Live Scores</button>
                       <button onClick={() => setViewMode('news')} className={`flex-1 md:flex-none h-12 md:h-full px-6 text-[10px] font-bold uppercase tracking-widest border-r border-gray-100 transition-colors ${viewMode === 'news' ? 'bg-black text-white' : 'text-gray-500 hover:text-black hover:bg-gray-50'}`}>News & Reports</button>
                   </>
               )}
               <button onClick={() => window.location.reload()} className="h-12 md:h-full px-6 text-gray-500 hover:text-black border-r border-gray-100"><i className="fa-solid fa-rotate-right"></i></button>
               <button onClick={() => isAdmin ? setIsAdmin(false) : setShowLogin(true)} className="flex-1 md:flex-none h-12 md:h-full px-8 text-[10px] font-bold bg-penrice-gold text-black hover:bg-black hover:text-white transition-colors uppercase tracking-widest">{isAdmin ? 'Logout' : 'Staff Login'}</button>
           </div>
        </div>
      </header>

      <main className="flex-grow max-w-5xl mx-auto w-full px-4 lg:px-0 py-8 md:py-12 relative">
          {loading && <div className="absolute inset-0 flex items-start justify-center pt-20 bg-white/80 z-20 backdrop-blur-[2px]"><div className="text-2xl font-display font-bold text-black uppercase tracking-tight animate-pulse">Loading Fixtures...</div></div>}
                 ...
                 {viewMode === 'live' ? (
                     <div className="space-y-6">
                         <div className="mb-8 space-y-6">
                            <div className="flex flex-col md:flex-row justify-between items-center md:items-end border-b-2 border-black pb-4 gap-6">
                                <div className="text-center md:text-left w-full md:w-auto">
                                    <span className="text-lg md:text-sm font-bold text-black uppercase tracking-widest">Today's Fixtures</span>
                                     <div className="text-xs md:text-[10px] font-mono text-gray-400 mt-1">Showing {filteredMatches.length} {sportFilter === 'all' ? '' : sportFilter} Match{filteredMatches.length !== 1 ? 'es' : ''}</div>
                                </div>
                                <div className="w-full md:w-auto grid grid-cols-4 gap-2 md:flex md:gap-1">
                                    {['all', 'LIVE', 'UPCOMING', 'FT'].map(s => (
                                        <button key={s} onClick={() => setStatusFilter(s as any)} className={`flex items-center justify-center px-2 py-3 md:py-1 text-[10px] md:text-[10px] font-bold uppercase tracking-widest transition-colors rounded-sm border ${statusFilter === s ? 'bg-black text-white border-black' : 'bg-gray-50 md:bg-transparent text-gray-500 border-gray-100 md:border-transparent hover:text-black hover:bg-gray-200'}`}>{s === 'all' ? 'All' : (s === 'FT' ? 'Results' : s)}</button>
                                    ))}
                                </div>
                            </div>
                            <div className="grid grid-cols-2 md:flex gap-3 md:justify-start">
                                 {['all', 'football', 'cricket', 'netball', 'rugby'].map(s => (
                                     <button key={s} onClick={() => setSportFilter(s as any)} className={`flex items-center justify-center gap-2 px-4 py-4 md:py-2 border rounded-sm transition-all text-xs md:text-[10px] font-bold uppercase tracking-widest ${sportFilter === s ? 'bg-penrice-gold border-penrice-gold text-black shadow-sm' : 'bg-white border-gray-200 text-gray-500 hover:border-black hover:text-black'}`}>
                                         {s === 'all' && <i className="fa-solid fa-layer-group"></i>}
                                         {s === 'cricket' && <span>🏏</span>}
                                         {s === 'netball' && <span>🏐</span>}
                                         {s === 'rugby' && <span>🏉</span>}
                                         {s === 'football' && <span>⚽</span>}
                                         {s === 'all' ? 'All Sports' : s}
                                     </button>
                                 ))}
                            </div>
                         ...
                         {filteredMatches.map(match => {
                            ...
                            if (match.sport === 'football') return <FootballCard key={match.id} match={match} allMatches={matches} />;
      ...
      {showLogin && (
          <div className="fixed inset-0 bg-white/95 z-[100] flex items-center justify-center p-4 animate-fade-in">
              <div className="w-full max-w-sm text-center border border-black p-8 card-shadow-hard bg-white animate-pop-in">
                  ...
                  <input type="password" value={pin} onChange={(e) => setPin(e.target.value)} className={`w-full text-center text-3xl font-display font-bold tracking-[0.5em] border-b-2 py-4 mb-8 outline-none bg-transparent ${error ? 'border-red-600 text-red-600' : 'border-black'}`} placeholder="••••" autoFocus />
